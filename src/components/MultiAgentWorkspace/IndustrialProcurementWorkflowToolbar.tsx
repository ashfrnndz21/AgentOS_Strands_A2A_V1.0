...
import { Play, Square, Download, Shield, AlertTriangle, Factory, TrendingUp, DollarSign } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';

interface IndustrialProcurementWorkflowToolbarProps {
  isRunning: boolean;
  onRun: () => void;
  onStop: () => void;
  onExport: () => void;
...
export const IndustrialProcurementWorkflowToolbar: React.FC<IndustrialProcurementWorkflowToolbarProps> = ({
  isRunning,
  onRun,
  onStop,
  onExport,
  ...
  return (
    <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between bg-slate-800/90 backdrop-blur-lg border border-orange-400/20 rounded-xl px-3 py-2 shadow-lg">
      {/* Left Section - Title & Status */}
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <Factory className="h-5 w-5 text-orange-400" />
          <div>
            <h2 className="text-sm font-semibold text-white">Industrial Procurement</h2>
            <p className="text-xs text-gray-400">Supply Chain Optimization Workflow</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 text-xs">
          <div className="flex items-center gap-1">
            ...
            <span className="text-gray-300">{isRunning ? 'Running' : 'Ready'}</span>
          </div>
          <span className="text-gray-400">Agents: <span className="text-white font-medium">{nodeCount}</span></span>
          <span className="text-gray-400">Links: <span className="text-white font-medium">{connectionCount}</span></span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs font-semibold text-green-400">{metrics.complianceScore}%</span>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs font-semibold text-blue-400">{metrics.performanceScore}%</span>
        </div>
