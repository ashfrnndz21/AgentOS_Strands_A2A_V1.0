...
import { Play, Square, RotateCcw, Share2, Download, Upload, Shield, TrendingUp, FileCheck, BarChart3, Users, DollarSign } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Separator } from '@/components/ui/separator';
import { Badge } from '@/components/ui/badge';

interface TelcoCvmWorkflowToolbarProps {
  isRunning: boolean;
  onRun: () => void;
  onStop: () => void;
  onExport: () => void;
...
export const TelcoCvmWorkflowToolbar: React.FC<TelcoCvmWorkflowToolbarProps> = ({
  isRunning,
  onRun,
  onStop,
  onExport,
  ...
  return (
    <div className="absolute top-1 left-1 z-10 bg-slate-800/40 backdrop-blur-sm border border-slate-600/30 rounded-lg p-1.5 shadow-lg min-w-[600px]">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-[10px]">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-green-400">{metrics.revenueImpact}</span>
            </div>

            <div className="flex items-center gap-1">
              <Users className="h-2.5 w-2.5 text-cyan-400" />
              <span className="text-slate-300">Customers:</span>
              <span className="text-cyan-400">{metrics.customersAnalyzed.toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 text-[10px] text-slate-300">
            ...
            <span>{metrics.campaignsOptimized} Campaigns</span>
            {isRunning && (
