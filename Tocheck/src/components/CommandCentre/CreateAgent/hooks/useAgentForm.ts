...
export const useAgentForm = (onOpenChange: (open: boolean) => void) => {
  ...
  // RAG handlers
  const handleRAGEnabledChange = useCallback((enabled: boolean) => {
    form.setValue('ragEnabled', enabled);
    if (!enabled) {
      form.setValue('knowledgeBases', []);
    }
  }, [form]);

  const handleKnowledgeBasesChange = useCallback((bases: string[]) => {
    form.setValue('knowledgeBases', bases);
  }, [form]);

  const handleRAGConfigChange = useCallback((config: AgentFormValues['ragConfig']) => {
    form.setValue('ragConfig', config);
  }, [form]);
