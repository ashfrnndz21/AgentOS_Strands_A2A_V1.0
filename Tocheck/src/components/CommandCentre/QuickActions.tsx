...
import { OllamaTerminalDialog, OllamaModelsDialog } from '@/components/Ollama';
...
export const QuickActions: React.FC = () => {
  ...
  // Ollama handlers - Open dialogs for quick access
  ...
  const handleCreateOllamaAgent = useCallback(() => {
    // Open agent creation with Ollama pre-selected
    setCreateAgentOpen(true);
  }, []);

  const handleCreateStrandsOllamaAgent = useCallback(() => {
    // Redirect to Strands dashboard
    window.open('/strands-ollama-agents', '_self');
  }, []);
    ...
    <>
      ...
      <DropdownMenu>
        ...
        >
          ...
          <DropdownMenuItem 
            className="flex items-center gap-2 py-2.5 px-3 focus:bg-beam-blue/20 rounded-md cursor-pointer"
            onClick={handleCreateOllamaAgent}
          >
            <Cpu size={16} className="text-green-400" />
            <span>Create Ollama Agent</span>
          </DropdownMenuItem>
          ...
          >
            ...
            <div className="ml-auto">
              <div className="px-1.5 py-0.5 bg-purple-500/20 text-purple-300 text-xs rounded">
                Advanced
              </div>
            </div>
          </DropdownMenuItem>
          ...
          {/* Local AI Section - TEST */}
          <DropdownMenuLabel className="text-xs text-red-400 px-3 py-1">
            🔥 LOCAL AI TEST (Ollama)
          </DropdownMenuLabel>
          
          <DropdownMenuItem 
            className="flex items-center gap-2 py-2.5 px-3 focus:bg-beam-blue/20 rounded-md cursor-pointer"
            onClick={handleOllamaTerminal}
          >
          ...
          <DropdownMenuItem 
            className="flex items-center gap-2 py-2.5 px-3 focus:bg-beam-blue/20 rounded-md cursor-pointer"
            onClick={() => window.open('/ollama-terminal', '_blank')}
          >
            <Cpu size={16} className="text-purple-400" />
            <span>Full Terminal Page</span>
          </DropdownMenuItem>

          <DropdownMenuSeparator className="bg-gray-700 my-2" />

          {/* Document Intelligence Section */}
          <DropdownMenuLabel className="text-xs text-purple-400 px-3 py-1">
      ...
      <OllamaTerminalDialog 
        open={ollamaTerminalOpen} 
        onOpenChange={setOllamaTerminalOpen}
      />
      
      <OllamaModelsDialog 
        open={ollamaModelsOpen} 
        onOpenChange={setOllamaModelsOpen}
      />
    </>
  );
};