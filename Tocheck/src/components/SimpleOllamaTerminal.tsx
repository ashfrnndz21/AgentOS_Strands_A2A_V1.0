...
export const SimpleOllamaTerminal: React.FC = () => {
    ...
    const testConnection = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Test Ollama status
          const ollamaResponse = await fetch('http://localhost:5002/api/ollama/status');
          ...
          addLine('error', '❌ Backend responded with error');
        }
      } catch (error) {
        setConnectionStatus('error');
        addLine('error', '❌ Cannot connect to backend');
        addLine('info', '💡 Make sure to run: ./start-ollama-backend.sh');
        addLine('info', '💡 Backend should be running on port 5001');
      }
    };
